...
<h1>FAQ</h1>
<button onclick="window.location = '{{route('faq.create')}}'"  class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
    Create Question
</button>
<button onclick="window.location = '{{route('faq.create')}}'"  class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
    Categories
...
@foreach($categories as $category)
    ...
    @foreach($allFaqs as $faq)
        @if($faq->categories === $category)
            ...
            @endif
        @else
        <p>There are no questions related to the theme {{$category}}.</p>
        @endif
    @endforeach
@endforeach
